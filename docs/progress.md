...
- 撤銷/重做加入佇列管理（對照原版 _animation_queue + _process_next_in_queue）：快速連續按鍵會依序排隊播放，不互相打斷。
- 選中動畫修正：播放前高亮該步滑塊組「當前位置」，提交後高亮「終點位置」，對照原版 _flash_move_selection。
## 動畫與縮放修正

- 滾輪縮放以鼠標位置為中心（縮放後同一世界點留在鼠標下）。
- 撤銷/重做動畫改用 move_info：只動「該步滑塊組」，不再整版各自亂動。
- 歷史快照記錄 move_info（對照原版），滑動動畫與選中動畫分開：滑動動畫控制移動與撤銷/重做位移，選中動畫控制該步滑塊組的短暫高亮。
## 再修正

- 實作「选中动画」：撤銷/重做時會短暫高亮實際移動過的格子（黃色描邊）。
- 開啟存檔可撤回：相容原版 save JSON（matrix/bounds 歷史快照）與體驗版 JSON；載入後保留原始 m/n，歷史可 undo。
- 計時中途按空格停止 = DNF（對照原版 ready→開始、running→DNF）。
- 謎題選單補上 step=3 的 3~6*6 ... 3~10*10。
- 虛擬鍵盤按鈕字型統一（inherit + normal，不再粗細不一）。
## 驗收後續修正

- 右側面板：速度條移到縮放條右邊，兩者並排於面板上方。
- 虛擬鍵盤與成績列表改為「可拖動」的浮動面板（標題列拖動、×關閉）。
- 虛擬鍵盤排版對照原版：↑/←↓→十字鍵、粘滞/撤銷/重做、跳到某步。
- 「選中動畫」開關生效：控制撤銷/重做過場動畫是否播放。
- 修正載入存檔後無法撤銷：deserialize 後恢復謎題原始 m/n，歷史快照可正常 undo。
- 新增測試：載入存檔後可撤銷。
## 體驗版細節修正

- 自訂謎題改為頁面內表單（不再用瀏覽器 prompt）。
- 未實裝功能（宏定義、其他設定等）在選單標註「未實裝」，提示下載正式版。
- 修正非全屏視窗右側面板被畫布擠出畫面（canvas 不再 width:100% 撐爆 flex）。
- 右側面板排版調整：縮放/速度滑條在上、六個開關在下。
- 修正 reset：原地重建復原版面並清空歷史（先前因替換物件導致 UI 無感）。
- 快捷鍵補上原版 Ctrl+X 重做（另保留 Ctrl+Y）。
- 虛擬鍵盤、成績面板維持浮動面板，由 F1/F3 或設置選單開關。
- 移動後保留縫隙/滑塊組選中，支援連續滑動不需每次重選。
## 界面重構（貼近原版）
